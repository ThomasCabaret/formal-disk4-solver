...
def _map_vertex_name(name: str, permutation: Tuple[int, int, int]) -> str:
    ...
    if not (len(name) == 3 and name[0] == "O"):
        raise ValueError(f"Unexpected C3 vertex name: {name}")
    return _pair_name(permutation[int(name[1])], permutation[int(name[2])])


def build_c3_map() -> PlanarMap:
    """Three congruent sectors; the non-degenerate contact graph is C3."""

    pieces = (
        PieceSpec("P0", ("Z", "O01", "O02"), True, "arc"),
        PieceSpec("P1", ("Z", "O12", "O01"), True, "arc"),
        PieceSpec("P2", ("Z", "O02", "O12"), True, "arc"),
    )
