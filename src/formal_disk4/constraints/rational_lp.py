...
class RationalSimplex:
    """Tiny exact two-phase simplex for A x <= b, x >= 0.

    It is intended for the very small angular feasibility systems generated by
    formal contour profiles.  All arithmetic uses ``fractions.Fraction``.
    """
    ...
    def __init__(
        ...
        for row_index in range(self.m):
            for column_index in range(self.n):
                self.D[row_index][column_index] = Fraction(A[row_index][column_index])
            self.D[row_index][self.n] = Fraction(-1)
            self.D[row_index][self.n + 1] = Fraction(b[row_index])
        for column_index in range(self.n):
            self.D[self.m][column_index] = -Fraction(c[column_index])
        self.D[self.m + 1][self.n] = Fraction(1)

    def _pivot(self, row: int, column: int) -> None:
        inverse = Fraction(1, 1) / self.D[row][column]
        for row_index in range(self.m + 2):
            if row_index == row:
                continue
            for column_index in range(self.n + 2):
                if column_index == column:
                    continue
                self.D[row_index][column_index] -= (
                    self.D[row][column_index]
                    * self.D[row_index][column]
                    * inverse
                )
        for column_index in range(self.n + 2):
            if column_index != column:
                self.D[row][column_index] *= inverse
        for row_index in range(self.m + 2):
            if row_index != row:
                self.D[row_index][column] *= -inverse
    ...
    def solve(self) -> LinearProgramResult:
        if self.m == 0:
            ...
            return LinearProgramResult(
                "optimal",
                Fraction(0),
                tuple(Fraction(0) for _ in range(self.n)),
            )
        ...
        for row_index in range(self.m):
            if 0 <= self.B[row_index] < self.n:
                solution[self.B[row_index]] = self.D[row_index][self.n + 1]
        return LinearProgramResult(
            "optimal",
