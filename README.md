...
Version 1.1.0 extends the project around explicit **case identifiers**.  A
...
| `double-cycle-6` | Twelve tiles: exterior and interior `C6` cycles joined by matching `E_i-I_i` contacts. |
...
## Parameterized double-cycle family

`double-cycle-6` is the ready-to-run twelve-tile case.  Its map is generated by
`build_double_cycle_map(N)`, and the registry also accepts dynamic identifiers
`double-cycle-N` for every `N >= 3`.  To create an isolated case directory for
another size:
...
The generated case receives its own `output/cases/double-cycle-N/` directory
and SQLite checkpoint.  The `N=6` search uses `symmetry_mode=off` together with
random-access lazy assignments: the raw domain contains 6,115,295,232
phase/reflection assignments and is never materialized in memory.  Exact weak-
order domain counting is disabled for this case because the counting DP itself
would be prohibitive; all actual feasibility pruning remains enabled.

The encoded planar topology has:

- outer tiles `E1,...,EN` in a cycle, each owning one outer arc;
- inner tiles `I1,...,IN` in a cycle;
- one matching interface `Ei-Ii` for every index;
- a four-tile interior vertex for `Ei,E(i+1),Ii,I(i+1)`;
- one central point incident to all inner tiles.

No tile is marked as containing the disk centre strictly.
...
.venv\Scripts\python.exe -m formal_disk4 map-info --map double-cycle-9
