...
$LogFile = Join-Path $LogDirectory "pizza4_full_pipeline_$Timestamp.log"
...
try {
    ...
    Write-Host "============================================================"
    Write-Host "Formal Contour Solver - Complete Four-Piece Pizza Pipeline"
    ...
    $FormalArguments = @("-m", "formal_disk4", "run", "--config", "config\pizza4.json")
    if ($Restart) { $FormalArguments += "--restart" }
    Write-Host ""
    Write-Host "[STEP 1/3] Searching for a decorated formal four-piece pizza profile..."
    ...
    $GeometryArguments = @("-m", "formal_disk4", "geometry", "--config", "config\pizza4_geometry.json")
    ...
    $VisualizerArguments = @("-m", "formal_disk4", "visualize", "--config", "config\pizza4_visualizer.json")
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "[SUCCESS] Complete four-piece pizza pipeline finished successfully."
}
catch {
    if ($ExitCode -eq 0) { $ExitCode = 8 }
    Write-Host ""
    Write-Host "[ERROR] Complete four-piece pizza pipeline failed." -ForegroundColor Red
