...
$LogFile = Join-Path $LogDirectory "run_${ConfigStem}_$Timestamp.log"
...
try {
    ...
    Write-Host "============================================================"
    Write-Host "Formal Contour Solver - Pipeline Run"
    ...
    $Arguments = @("-m", "formal_disk4", "run", "--config", $Config)
    if ($Output) {
    ...
    Write-Host ""
    Write-Host "[INFO] Starting solver pipeline..."
    ...
    Write-Host ""
    if ($ExitCode -eq 0) {
        Write-Host "[SUCCESS] Solver pipeline completed successfully."
    }
    else {
        Write-Host "[ERROR] Solver pipeline failed with exit code $ExitCode." -ForegroundColor Red
    }
}
catch {
    $ExitCode = 6
    ...
    Write-Host ""
    Write-Host "[INFO] Run script exit code: $ExitCode"
